...
export const useAdminWebPush = ({ enabled }) => {
    ...
    useEffect(() => {
        ...
        const tryEnable = async () => {
            ...
            try {
                ...
                // Ensure the SW is active/ready before using PushManager.
                // Without this, some browsers can return null subscriptions right after install/update.
                try {
                ...
                // Best-effort; do not break admin UX
            }
        };

        // If already granted, we can subscribe immediately (no user gesture needed)
        if (Notification.permission === "granted") {
            tryEnable();
            return;
        }

        // Otherwise, browsers require a user gesture for requestPermission.
