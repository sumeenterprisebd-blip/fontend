        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <section className="rounded-2xl border border-gray-100 bg-white p-5 sm:p-6">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900">Size Guide</h2>
              <p className="mt-2 text-sm sm:text-base text-gray-700">
                Use the chart below to pick the best fit. If you’re between sizes, consider sizing up.
              </p>
              ...
              {measurementGroups.length > 0 ? (
                <div className="mt-4">
                  <div className="flex flex-wrap gap-2 border-b border-gray-200 pb-3">
                    {measurementGroups.map((group, index) => {
                      ...
                      const label = category?.name || group.category?.name || `Category ${index + 1}`;
                      return (
                        <button
                          key={String(group.category) || index}
                          type="button"
                          onClick={() => setActiveMeasurementIndex(index)}
                          className={`rounded-full px-4 py-2 text-sm font-semibold transition ${index === activeMeasurementIndex
                  ...
                  {activeMeasurementGroup ? (
                    <div className="mt-4 overflow-hidden rounded-xl border border-gray-100 bg-gray-50">
                      <div className="flex gap-2 overflow-x-auto px-4 py-3 text-[11px] sm:text-xs font-semibold text-gray-600 border-b border-gray-100">
                        <div className="min-w-[120px]">Size</div>
                        {(Array.isArray(activeMeasurementGroup.category?.fields) && activeMeasurementGroup.category.fields.length > 0
                          ? activeMeasurementGroup.category.fields
                          : Object.keys(activeMeasurementGroup.sizes?.[0]?.values || {})
                        ).map((field) => (
                          <div key={field} className="min-w-[120px] flex-1 text-center">
                            {field}
                          </div>
                        ))}
                      </div>

                      {Array.isArray(activeMeasurementGroup.sizes) && activeMeasurementGroup.sizes.length > 0 ? (
                        activeMeasurementGroup.sizes.map((row, rowIndex) => (
                          <div
                            key={`${row.size || 'row'}-${rowIndex}`}
                            className="flex gap-2 overflow-x-auto px-4 py-3 text-sm sm:text-base text-gray-900"
                          >
                            <div className="min-w-[120px] font-semibold">{row.size || '—'}</div>
                            {(Array.isArray(activeMeasurementGroup.category?.fields) && activeMeasurementGroup.category.fields.length > 0
                              ? activeMeasurementGroup.category.fields
                              : Object.keys(row.values || {})
                            ).map((field) => (
                              <div key={`${rowIndex}-${field}`} className="min-w-[120px] flex-1 text-center">
                                {row.values?.[field] || '–'}
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="px-4 py-4 text-sm text-gray-500">
                          No size rows have been added for this measurement category.
                        </div>
                      )}
                    </div>
                  ) : null}
                ...
                <div className="mt-4 rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
                  This product does not have a dedicated size guide. Use the available sizes below or add measurement details in the admin dashboard.
                </div>
              )}

              {Array.isArray(product?.sizes) && product.sizes.length > 0 && (
                <div className="mt-4">
                  <div className="text-xs sm:text-sm font-semibold text-gray-700">Available sizes</div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {product.sizes.map((size) => (
                      <span
                        key={String(size)}
                        className="inline-flex items-center rounded-full border border-gray-200 bg-white px-3 py-1 text-xs sm:text-sm font-semibold text-gray-900"
                      >
                        {size}
                      </span>
                    ))}
              ...
              <p className="mt-4 text-xs sm:text-sm text-gray-600">
                Tip: If you need help choosing a size, contact support with your height/weight for guidance.
              </p>
            </section>
